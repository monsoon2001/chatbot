...
const Admin: React.FC = () => {
  ...
  const handleUserClick = (user: User) => {
    if (selectedUser && selectedUser._id === user._id) {
      setSelectedUser(null); // Deselect user if the same user is clicked again
    } else {
      setSelectedUser(user); // Select the new user
    }
  };
        ...
        <div className="max-w-7xl mx-auto">
          ...
          {selectedUser && (
            <div>
              ...
              <div className="border p-4 rounded shadow bg-white text-black mb-4">
              ...
              <div className="mb-4">
                ...
                <div className="space-y-2">
                  {selectedUser.chats.map((chat: Chat) => (
                    <div key={chat._id} className="border p-2 rounded bg-gray-100">
                      <p className="text-gray-600"><strong>{chat.role}:</strong> {chat.content}</p>
                      <p className="text-xs text-gray-500">Time: {new Date(chat.createdAt).toLocaleString()}</p>
                    </div>
                  ))}
                </div>
              </div>
              <div>
                ...
                <div className="space-y-2 text-black">
                  {selectedUser.feedback.map((feedback: Feedback) => (
                    <div key={feedback._id} className="border p-2 rounded bg-gray-100">
